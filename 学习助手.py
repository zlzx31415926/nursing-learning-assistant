...
【输出精简规则——为防截断，必须遵守】：
1. 答案直接写在各题后面，格式：`答案：①xxx ②xxx`。严禁在文末单独生成"## 本阶段答案"汇总段落。
2. 选择题不用"子题1：子题2："前缀，直接以题目正文开头，选项紧跟其后。
3. 每道选择题的解析控制在2-3行，不要展开长篇大论。
4. 能用表格对比的就不要用文字重复叙述。
...
- 不生成记忆口诀——用「机制反推导」替代
- 基础填空≥15空 | 案例选择≥6道 | 配对≥8组 | 坑点≥8条
- 不因考频低跳过任何知识点，一律平等对待

【选择题解析格式——正文不教的考试策略】：
正文教"是什么+为什么"，解析教"怎么考+为什么错+怎么排除"——两者不重复。
每个选项必须按此结构：
  ✗ 错误选项：
    → 为什么有人选它：（学生常犯错误心理）
    → 什么时候反而对：（边界条件——打破"绝对错"）
  ...
  ✓ 正确选项：
    ...
    → 🔑 锁定词：（题干哪几个词决定答案）
    → ⚠️ 边界：（什么情况下它反而错）
对照四种考试心理：泛化型（打破"首选=永远选"）、关键词陷阱（往回找限定词）、干扰成对（回题干找绝杀词）、优先级混淆（先简单后复杂）
...
每类选3-5个最核心的条目即可，不必面面俱到。"""
    ...
    # ========== 第三档：拆成三次，每阶段4000 tokens ==========
    if tier == 3:
        config_1_2 = """请生成阶段一至阶段二：
...
阶段四：应用 - 渐进决策模式（4步递进）+ 优先级排序（至少1道动手排序题，追问"为什么A在B前面"）+ 跨章节融合决策（引入合并症，药物选择×过敏史×合并症三角互动）"""
        ...
        config_5_6 = """请继续生成阶段五至阶段六：
阶段五：检验 - 高仿真送命题（4-5题含完整选项解析）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵 + 跨章节综合病例串（2-3个病例）+ 案例选择题×高难填空题配对（至少8组）+ 集中坑点预警清单（按类型分组，至少8条）
阶段六：复盘 - 学习环总览 + 阶段自检清单（每个阶段一个核心问题）+ 错题画像 + 知识修补清单 + 考前救急方案"""

        # 第一次：阶段1-2
        content_1_2 = call_deepseek(
            base_prompt + f"\n\n【生成内容】：{config_1_2}\n\n请输出 Markdown 格式的阶段一至二。",
            SYSTEM_PROMPT, max_tokens=8000
        )
        ctx1 = content_1_2[-600:] if len(content_1_2) > 600 else content_1_2

        # 第二次：阶段3-4
        content_3_4 = call_deepseek(
            base_prompt + f"\n\n【前文摘要】：{ctx1}\n\n【生成内容】：{config_3_4}\n\n请接着输出阶段三至四。保持与前文一致，不矛盾。",
            SYSTEM_PROMPT, max_tokens=8000
        )
        ctx2 = content_3_4[-600:] if len(content_3_4) > 600 else content_3_4

        # 第三次：阶段5-6
        try:
            content_5_6 = call_deepseek(
                base_prompt + f"\n\n【前文摘要】：{ctx2}\n\n【生成内容】：{config_5_6}\n\n请接着输出阶段五至六。保持与前文一致。",
                SYSTEM_PROMPT, max_tokens=8000
            )
        except Exception:
            content_5_6 = "\n\n---\n\n⚠️ 阶段五至六生成失败，请点「🔄 重新生成」重试。\n\n"

        return content_1_2 + "\n\n" + content_3_4 + "\n\n" + content_5_6

    # ========== 第二档：拆成两次 ==========
    if tier == 2:
        t2_config_a = """请生成阶段一至阶段三（简化版）：
阶段一：理解——「一句话速记」+ 核心推理链（1-2条，选最关键的，每条必须延伸到🩺护理行动）+「机制反推导」（每条配1道反推题）+「关键概念解剖」1-2个+「机制→表现对应表」
阶段二：记忆——基础填空（至少10空）+ 对比速记表 + 审题关键词速查
阶段三：辨析——细节辨析（2-3个陷阱，标注干扰项设计意图）+ 命名陷阱预判（至少2个）+ 机制反推导"""

        t2_config_b = """请继续生成阶段四至阶段六（简化版）：
阶段四：应用——渐进决策模式（4步递进）+ 优先级排序（1道动手排序题）+ 跨章节融合决策（含1个合并症）
阶段五：检验——高仿真送命题（2-3题含完整解析）+ 跨章节变形题（1-2道）+ 案例选择题×高难填空题配对（至少5组）+ 集中坑点预警清单（至少6条）
...
不生成完整推理链、送命题、渐进决策。"""
        ...
        return call_deepseek(prompt, SYSTEM_PROMPT, max_tokens=4000, progress_placeholder=progress_placeholder)
