...
阶段五：检验 - 高仿真送命题（4-5题）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵 + 跨章节综合病例串（2-3个病例）+ 案例选择题×高难填空题配对（至少8组）+ 集中坑点预警清单（至少8条）
⚠️阶段五选择题解析格式（仅阶段五使用）：
  ✗错误选项：→为什么有人选它 →什么时候反而对 →为什么这里错 →🔑排除法
  ✓正确选项：→理由 →🔑锁定词 →⚠️边界
阶段六：复盘 - 学习环总览 + 阶段自检清单 + 错题画像 + 知识修补清单 + 考前救急方案"""
