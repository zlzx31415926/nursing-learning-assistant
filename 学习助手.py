...
# AI 引擎
# ============================================================
SYSTEM_PROMPT = """你是一位资深护理考试命题专家。你的任务是根据提供的原始知识点，生成高质量的结构化学习材料。

⚠️ 你的首要职责不是出题，而是帮零基础学生"彻底理解"这个疾病。阶段一（理解）是地基——占40%篇幅，之后五个阶段才有依据。
...
双病例对比题格式：患者A和患者B同时就诊，各自诊断？处理？谁更紧急？"""


def ai_group_and_sort(raw_text: str) -> dict:
    """AI 归组：从散乱原始资料中识别疾病并按逻辑排序。"""
    sample = raw_text[:8000] if len(raw_text) > 8000 else raw_text
    prompt = f"""以下是一份护理学知识点的原始资料（前8000字符样本）。资料中的知识点是散乱排列的，同一疾病分散在多处，不同疾病混杂在一起。
...
{{
  "diseases": [
    {{
      ...
      "confidence": "high",
      "entry_count": 18,
      "original_entries": ["条目66", "条目76", "条目77", ...],
      "merged_entries": [["条目76", "条目87", "两者都讲最常见心律失常"]],
...
- 基础填空 + GU/DU式对比速记表
- 审题关键词速查
- 细节辨析（2-3个高频陷阱，每个含"为什么错"解释）
- 出题人陷阱预判（至少2个命名陷阱）
- 渐进决策模式（分两部分：基础版4步递进 + 🔥跨章节融合版含1个合并症）
- 高仿真送命题（分两部分：标准版2-3子题 + 🔥DeepSeek融合版1个跨科病例含命名陷阱）
- 1道跨章节变形题
- 坑点预警清单
- 关键词提取矩阵""",

        3: """第三档（核武器）：生成完整六阶段学习环：
阶段一：理解（⚠️地基，占40%篇幅）——「一句话速记」+「图解式推理链」至少4-5条（每条7-15步，每步解释"为什么"）+「关键概念解剖」2-3个（通俗比喻）+「机制→表现对应表」（两列：机制|表现+标注重要性★）
阶段二：记忆 - 基础填空 + 对比速记表 + 图解记忆法 + 关键词提取矩阵 + 审题关键词速查
阶段三：辨析 - 鉴别诊断对比表 + 细节辨析（3-5个陷阱含主动预判）+ 命名陷阱预判（至少3个）+ 反向归类
阶段四：应用 - 分两部分：
  【第一部分·基础临床决策】标准的渐进决策模式（4步递进：评估→判断→处理→预防）+ 优先级排序 + 风险识别
  【第二部分·跨章节融合决策】引入合并症（糖尿病/高血压/肾衰/肝病），追问合并症对处理的交互影响。药物选择×过敏史×合并症的三角互动。血糖高能否推迟手术？肾功能不全对抗生素选择的影响？
阶段五：检验 - 分两部分：
  【第一部分·标准送命题】高仿真送命题（3-4子题含解析）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵
  【第二部分·DeepSeek融合案例】跨章节综合病例串（2-3个病例）。每个病例自带合并症。包含：双病例对比题、药物选择×合并症×过敏史三角互动、命名陷阱（🔥陷阱N：标题+类型+干扰项+破题关键）。坑点预警清单（8-10条，按类型分组）
  【第三部分·案例选择题×高难填空题配对】至少8组，越多越好。每组：左边案例选择题（病例题干+ABCD+解析）+ 右边对应考点的高难度填空题（挖掉数值/机制/禁忌词）。一一配对，用两列表格呈现。
阶段六：复盘 - 学习环总览 + 错题画像 + 知识修补清单 + 考前救急方案"""
    }
    prompt = f"""请为"{disease_name}"生成一份护理考试学习材料。

⚠️ 阶段一是地基——零基础学生全靠它理解疾病本质。阶段一占整篇40%篇幅，之后五个阶段才有依据。
...
2. 「图解式推理链」：至少4-5条，每条7-15步，每步用括号解释"为什么"
   格式：A→B（因为...）→C（导致...）→D
   每条链末尾必加「💡 关键追问：...」
3. 「关键概念解剖」：挑3个核心名词/解剖/生理，各80-120字通俗解释+生活比喻
4. 「机制→表现对应表」：两列 | 机制（为什么）| 临床表现（所以会怎样）|，每个标★

【输出质量铁律——以下三条不遵守则整篇作废】：
1. 每道选择题必须有：完整的病例题干（至少2行）+ 明确的问题 + 4个完整选项 + 每个选项的逐一解析
   ❌ 禁止：只有选项没有题干 / 题干只有一句话 / 解析只写"对"或"错"
2. 内容不得截断——如果字数不够，宁可少生成1道题，也要把已有的题写完整
3. 阶段五末尾的配对表，每行左右两列都必须有完整内容，不能出现空单元格

【阶段二至六·统一要求】：
- 每个知识点用"### 知识点N：xxx"独立成段，不合并
- 每个知识点末尾标：> 📌 考试重要性：★★★~★ | 考查形式：选择/填空/病例
- 填空______①______连续编号，答案放"## 本阶段答案"
- 基础填空至少15空 | 案例选择至少6道（完整解析）| 配对至少6组 | 坑点至少6条
- 不因考频低而跳过任何知识点
            ...
            with cols[0]:
                if st.button(f"📄 {fname}（{fsize:.0f}KB）", key=f"load_{sf.stem}", use_container_width=True,
                    ...
                    with st.spinner("🤖 AI 正在识别疾病并归组..."):
                        try:
                            result = ai_group_and_sort(raw_text)
