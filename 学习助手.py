...
阶段一：理解（⚠️机制地基）——「一句话速记」+「图解式推理链」至少4-5条（每条7-15步，每步解释"为什么"，末尾必加🩺护理行动延伸：我看到→我想到→我决定）+「机制反推导」每条推理链配1道反推题（给表现→反推机制→排除其他→如果相反）+「关键概念解剖」2-3个（通俗比喻）+「机制→表现对应表」（两列：机制|表现+标注重要性★）
阶段二：记忆 - 基础填空（至少15空，因果对形式："因为A→所以B"配对）+ 对比速记表 + 图解记忆法 + 关键词提取矩阵 + 审题关键词速查（"看到X→立刻锁定Y"快速反应格式）
阶段三：辨析 - 鉴别诊断动态分叉推导（从共同症状出发→分叉鉴别→落到不同疾病，训练"凭什么区分"而非"区分结果是什么"）+ 细节辨析（3-5个陷阱含主动预判，每个标注干扰项设计意图："为什么有人会选错"）+ 命名陷阱预判（至少3个，含类型+干扰项+破题关键）+ 反向归类 + ⭐⭐ 机制反推导（给表现→反推机制→排除其他诊断）
阶段四：应用 - 分两部分：
  【第一部分·基础临床决策】标准的渐进决策模式（4步递进：评估→判断→处理→预防）+ 优先级排序（至少1道动手排序题，多项措施排顺序，追问"为什么A在B前面"，必须解释每项逻辑）+ 风险识别
  【第二部分·跨章节融合决策】引入合并症（糖尿病/高血压/肾衰/肝病），追问合并症对处理的交互影响。药物选择×过敏史×合并症的三角互动。血糖高能否推迟手术？肾功能不全对抗生素选择的影响？
阶段五：检验 - 分三部分：
  【第一部分·标准送命题】高仿真送命题（4-5子题含解析）+ 每个干扰项标注设计意图（成人思维干扰/数值互换/概念颠倒/药物张冠李戴）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵
  【第二部分·DeepSeek融合案例】跨章节综合病例串（2-3个病例）。每个病例自带合并症。包含：双病例对比题、药物选择×合并症×过敏史三角互动、命名陷阱（🔥陷阱N：标题+类型+干扰项+破题关键）
  【第三部分·案例选择题×高难填空题配对】至少8组，越多越好。每组：左边案例选择题（完整病例题干+ABCD+逐项解析+干扰项设计意图）+ 右边对应考点高难度填空题（挖掉数值/机制/禁忌词）。一一配对，用两列表格呈现。
  【集中坑点预警清单】按类型分组（数值混淆/概念颠倒/药物张冠李戴/禁忌忽略/并发症互换/优先级错误），每行：坑点描述 | 为什么容易错 | 正确做法。至少8条。考前快速扫描用。
阶段六：复盘 - 学习环总览 + 阶段自检清单（每个阶段一个核心问题，确认掌握）+ 错题画像 + 知识修补清单 + 考前救急方案"""
    }
    prompt = f"""请为"{disease_name}"生成一份护理考试学习材料。你的视角是**护士推理**——不是"老师讲给你听"，而是训练学生"看到病人→推出诊断→决定护理优先级"。
...
2. 「图解式推理链」：至少4-5条，每条7-15步，每步用括号解释"为什么"
   格式：A→B（因为...）→C（导致...）→D
   每条链末尾必加「💡 关键追问：...」+「🩺 护理行动：我看到___→我想到___→我决定___」（推理链必须延伸到护理决策，不能止于病理生理）
3. 「机制反推导」：每条推理链配1道反推题
...
【输出质量铁律——以下不遵守则整篇作废】：
1. 每道选择题必须有：完整病例题干（≥2行）+ 明确问题 + 4个完整选项 + 逐一解析 + 每个干扰项标注设计意图（为什么有人会选这个）
   ❌ 禁止：无题干 / 题干一句话 / 解析只写"对""错"
2. 内容不得截断——字不够就少生成题，但不能把题写一半
3. 配对表左右两列都必须是完整内容，不能出现空单元格

【阶段二至六·统一要求】：
- 每个知识点独立成段，不合并；末尾标重要性★和考查形式
- 鉴别诊断用**动态分叉推导**：从共同症状出发→分叉鉴别点→落到不同诊断，训练"凭什么区分"
- 送命题每个干扰项标注**设计意图**：成人思维干扰/数值互换/概念颠倒/药物张冠李戴/并发症互换
- 优先级排序题**必须让学生动手排**，追问"为什么A在B前面"
- 坑点预警清单**集中按类型分组**，每行：坑点 | 为什么容易错 | 正确做法；方便考前扫描
- 不生成记忆口诀——用「机制反推导」替代
