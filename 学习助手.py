...
阶段六：复盘 - 学习环总览 + 阶段自检清单 + 错题画像 + 知识修补清单 + 考前救急方案"""
        ...
        if progress_placeholder is not None:
            progress_placeholder.info("📝 正在生成完整六阶段...")
        draft = call_deepseek(
            base_prompt + f"\n\n【生成内容】：{full_config}\n\n请输出完整的 Markdown 格式六阶段学习材料。",
            SYSTEM_PROMPT, max_tokens=8000
        )

        # 第二次：精修——拿全文做审查扩充
        if progress_placeholder is not None:
            progress_placeholder.info("🔍 正在审查精修：修正矛盾、补充遗漏、深化弱项...")
        refine_prompt = f"""以下是一份已完成的学习材料。请你作为审稿专家，对它进行全面审查和扩充。

审查清单（逐一检查并修正）：
1. **矛盾检查**：六个阶段之间是否存在前后矛盾？（比如阶段二说首选A药，阶段四却用B药；阶段一说"常见于青壮年"，阶段三却说"多见于老年人"）→ 发现矛盾立即修正。
2. **数量达标**：填空是否≥15空？案例选择题是否≥6道且有完整解析+干扰项设计意图？配对是否≥8组？坑点是否≥8条？→ 不达标的补齐。
3. **推理链完整性**：每条推理链是否延伸到护理决策（🩺 护理行动）？没延伸的补上。
4. **送命题干扰项**：每个错误选项是否标注了设计意图（为什么有人会选这个）？没标的补上。
5. **优先级排序题**：是否解释了"为什么A排在B前面"？没有解释的补上。
6. **弱项深化**：扫描全篇，找出任何写得太浅、一笔带过的地方，展开深化。

⚠️ 输出要求：
- 输出完整的六阶段材料，不是只输出修改意见
- 保持原有的 Markdown 格式和阶段标题（# 阶段一：理解 ~ # 阶段六：复盘）
- 如果原文质量已经很好，就在原文基础上补强，不要推翻重写

原文：
{draft}"""
        try:
            refined = call_deepseek(refine_prompt, SYSTEM_PROMPT, max_tokens=8000)
            return refined
        except Exception:
            # 精修失败时返回初稿
            return draft
