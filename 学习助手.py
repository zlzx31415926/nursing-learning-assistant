...
阶段五：检验 - 高仿真送命题（3-4子题含解析）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵 + 案例选择题×高难填空题配对（至少6组）+ 坑点预警清单（8-10条，按类型分组）
阶段六：复盘 - ①学习环总览（六阶段核心知识网络图） ②易错知识点汇总表（本疾病最容易失分的5-8个知识点，每行：易错点|错误原因|正确理解|🔑记忆技巧） ③考试思路总结（从题干关键词→锁定疾病→鉴别排除→确定答案的完整解题路径，200-300字） ④考前速查清单（数值/首选药/禁忌/特征性表现 四列速查表） ⑤知识修补清单（本疾病学习中可能遗漏的边角知识点）"""
...
阶段五：检验——高仿真送命题（2-3题）+ 跨章节变形题（1-2道）+ 案例选择题×高难填空题配对（至少5组）+ 集中坑点预警清单（至少6条）
⚠️阶段五选择题解析格式（仅阶段五使用）：✗错误选项：→为什么有人选它 →什么时候反而对 →为什么这里错 →🔑排除法 | ✓正确选项：→理由 →🔑锁定词 →⚠️边界
阶段六：复盘——学习环总览 + 易错知识点汇总表（易错点|错误原因|正确理解|记忆技巧）+ 考试思路总结（题干关键词→锁定疾病→鉴别排除→确定答案）+ 考前速查清单（数值/首选药/禁忌/特征性表现 四列表）+ 知识修补清单"""
