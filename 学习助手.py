...
阶段三：辨析 - 鉴别诊断动态分叉推导 + 细节辨析（3-5个陷阱，标注干扰项设计意图）+ 命名陷阱预判（至少3个）+ 反向归类 + 机制反推导"""
        ...
        stages_4_6_config = """请继续生成阶段四至阶段六：
阶段四：应用 - 分两部分：
  【基础临床决策】渐进决策模式（4步递进）+ 优先级排序（至少1道动手排序题，追问"为什么A在B前面"）+ 风险识别
  【跨章节融合决策】引入合并症（糖尿病/高血压/肾衰/肝病），药物选择×过敏史×合并症三角互动
阶段五：检验 - 分三部分：
  【标准送命题】高仿真送命题（4-5子题含解析，干扰项标注设计意图）+ 跨章节变形题（2-3道）+ 跨考点串联矩阵
  【DeepSeek融合案例】跨章节综合病例串（2-3个病例，自带合并症、双病例对比、命名陷阱）
  【案例选择题×高难填空题配对】至少8组：左边完整病例选择题 + 右边对应高难度填空题
  【集中坑点预警清单】按类型分组，至少8条
阶段六：复盘 - 学习环总览 + 阶段自检清单（每个阶段一个核心问题）+ 错题画像 + 知识修补清单 + 考前救急方案"""
        ...
        content_1_3 = call_deepseek(
            base_prompt + f"\n\n【生成内容】：{stages_1_3_config}\n\n请输出 Markdown 格式的阶段一至三。",
            SYSTEM_PROMPT, max_tokens=8000
        )

        # 第二次调用：生成阶段4-6，用前三个阶段的摘要作为上下文
        if progress_placeholder is not None:
            progress_placeholder.info("📝 正在生成阶段四至六（应用·检验·复盘）...")
        context_1_3 = content_1_3[-800:] if len(content_1_3) > 800 else content_1_3
        try:
            content_4_6 = call_deepseek(
                base_prompt + f"\n\n【前三个阶段已生成的内容摘要】：\n{context_1_3}\n\n【生成内容】：{stages_4_6_config}\n\n请接着输出 Markdown 格式的阶段四至六。",
                SYSTEM_PROMPT, max_tokens=8000
            )
        except Exception:
            content_4_6 = "\n\n---\n\n⚠️ 阶段四至六生成失败，请重新生成。\n\n"
